...
export function extractReplayId(url) {
    const match = url.match(/replay\.pokemonshowdown\.com\/([a-zA-Z0-9-]+)/);
    return match && match[1] ? match[1] : null;
}
const threadCache = new Map();
async function fetchAllReplaysFromThread(baseUrl) {
    let cleanUrl = baseUrl.replace(/\/page-\d+/, "");
    cleanUrl = cleanUrl.replace(/#.+/, "");
    if (!cleanUrl.endsWith('/'))
        cleanUrl += '/';
    if (threadCache.has(cleanUrl))
        return threadCache.get(cleanUrl);
    const promise = (async () => {
        const allReplays = [];
        let page = 1;
        let lastFirstPostID = "";
        while (true) {
            ...
            if (firstPostID === lastFirstPostID)
                break;
            lastFirstPostID = firstPostID;
            const links = Array.from(doc.querySelectorAll("a[href*='replay.pokemonshowdown.com']"));
...
export async function loadReplays(urls, isPreload = false) {
    if (!urls[0])
        return [];
    if (isSmogon(urls[0])) {
        if (urls.length > 1)
            throw new Error("One smogon URL at a time please...");
...
export async function groupReplays(replays, config) {
    ...
    for (const [key, group] of groups) {
        ...
        for (const replay of group.replays) {
            if (!replay.winner)
                continue;
            if (replay.playerNames.p1 !== p1name)
                wins[replay.winner]--;
            else
                wins[replay.winner]++;
        }
        ...
        if (wins.p2 > wins.p1)
            group.winner = "p2";
        if (wins.p1 === wins.p2)
            throw new Error("It shouldn't be possible for both players to win the same amount of games in a boX.");
        groups.set(key, group);
    }
    return Array.from(groups.values());
}
export async function parseReplays(replays) {
    const replayData = [];
    for (const replay of replays) {
...
export function parseReplay(replayData) {
    const { id, log, format } = replayData;
    if (replayCache.has(id))
        return deepcopy(replayCache.get(id));
    const names = {
        p1: '',
        p2: ''
    };
    ...
    for (const line of log.split('\n')) {
        ...
        if (type === "switch") {
            const player = rest[0].match(/p[12]/)?.[0];
            const pokemon = rest[1].split(',')[0];
...
export function parseClicked(data) {
    return parseReplay(data).clicked;
}
export function getPlayerNames(data) {
    return parseReplay(data).playerNames;
}
export function getWinningPlayer(data) {
    return parseReplay(data).winner;
}
export function buildUsageData(data) {
