...
let bbcodeDebounceTimer = null;
export function renderMascotFields(teams) {
...
export function renderReplayData(ctx) {
...
function createGroupCard(group, index, ctx) {
...
function createReplayList(group) {
...
function createFormatRow(group, index, ctx) {
...
function createSwapButton(group, index, ctx) {
...
function createPlayerRow(group, index, player, ctx) {
...
function createSmogonInput(playerData, index, ctx) {
...
function createTeamSelect(playerData, ctx) {
    ...
    for (const team of ctx.config.teams) {
        ...
        if (playerData.team?.name === team.name)
    ...
    teamSelect.addEventListener("change", () => {
        updateGroup(ctx, () => {
            ...
            if (selected && playerData.smogon)
                setPlayerTeam(playerData.smogon, selected.name);
        });
    });
    return teamSelect;
}
function updateHeader(groupIndex, replayGroups) {
    const card = document.querySelector(`[data-group-index="${groupIndex}"]`);
    const header = card?.querySelector(".group-header");
    if (header)
        header.textContent = formatGroupHeader(replayGroups[groupIndex]);
}
...
function scheduleBBCodeUpdate(ctx) {
    if (bbcodeDebounceTimer)
        clearTimeout(bbcodeDebounceTimer);
...
function updateGroup(ctx, callback, groupIndex) {
    callback();
